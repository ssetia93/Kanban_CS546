...
router.post("/", (req, res) => {
	...
	taskData.otherAddTask(blogTaskData.title,blogTaskData.creator, blogTaskData.description, blogTaskData.duedate, blogTaskData.creationdate, blogTaskData.priority)
        .then((newTask) => {
            res.json(newTask);
        }).catch((e) => {
            res.status(500).json({ error: e });
